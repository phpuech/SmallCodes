...
#where are my files ? 
inputpath = '/home/as/Bureau/comparaison/'
...
print cas4
#creation plot
fig=plt.figure("comparaison", figsize=(5,7))

# creation liste des datas
data=[cas0['median'],cas1['median'],cas2['median'], cas3['median'], cas4['median']]
print data
#boxplots using sns

plt.boxplot(data)#, color=['blue', 'red', 'green', 'red', 'green'], saturation=[0,0.5,0.5,0.25,0.25])#,'green'#,'blue','orange'], widths=.5, alpha=0.5)
#sns.boxplot(data)#, color=valeurs)#, color=['red', 'red', 'green'])
#scatter plot using plt
#define vectors
x1=1*np.ones(len(data[0]))
x2=2*np.ones(len(data[1]))
x3=3*np.ones(len(data[2]))
x4=4*np.ones(len(data[3]))
x5=5*np.ones(len(data[4]))
...
plt.xticks([1,2,3,4,5], ["Thy1GFP-C", "600-C", "603-C", "600-S30", "603-S30"])
